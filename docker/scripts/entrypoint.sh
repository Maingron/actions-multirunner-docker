...
done < <(python3 /usr/local/bin/parse-config.py "$CONFIG_FILE")
unset __runner_line
...
if [[ -s "$RUNNER_STATE_FILE" ]]; then
    ...
    while IFS= read -r rec; do
        ...
        if python3 /usr/local/bin/delete-runner.py "$r_url" "$r_pat" "$r_id"; then
            stale_cleaned=$((stale_cleaned + 1))
            python3 /usr/local/bin/runner-store.py remove "$r_id" || true
        fi
    done < <(python3 /usr/local/bin/runner-store.py list)
...
# a dedicated `docker:dind` sidecar (rendered by render.py). The runner
...
if (( ANY_DOCKER == 1 )); then
    if [[ -z "${DOCKER_HOST:-}" ]]; then
        ...
        echo "entrypoint: re-run python3 ./docker/render.py + ./start.sh so the compose file is regenerated with the DinD sidecar." >&2
        exit 1
    fi
...
if (( ${#PKGS_TO_INSTALL[@]} > 0 )); then
    ...
    if sudo -n python3 /usr/local/bin/install-packages.py "${PKGS_TO_INSTALL[@]}"; then
...
# /runner-storage) via a docker named volume declared by render.py.
...
if (( ANY_PS == 1 )); then
    if [[ ! -d "$PERSISTENT_STORAGE_ROOT" ]]; then
        ...
        echo "entrypoint: re-run python3 ./docker/render.py + ./start.sh so the compose file is regenerated with the runner-storage volume." >&2
        exit 1
    fi
